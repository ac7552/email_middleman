...
require 'send_grid_api_service'
require 'mail_gun_api_service'

RSpec.describe EmailsController, :type => :controller do
  let(:proper_email) { {"email": {"from" => "Testing123Joe@gmail.com",
                        "to" => "Testing123Jack@gmail.com",
  ...
  describe "POST email with MailGun API" do
    before do
    ...
    it "returns 200" do
      MailGunApiService.any_instance.stub(:send_email).and_return(true)
  ...
  describe "POST email with SendGrid API" do
    before do
      adapter = Flipper::Adapters::ActiveRecord.new
      flipper = Flipper.new(adapter)
      flipper[:default_mailer].disable
    end

    it "returns 200" do
      SendGridApiService.any_instance.stub(:send_email).and_return(true)
      post(:create, params: proper_email)
      expect(response.status).to eq(200)
    end
    it "returns 422 when missing fields" do
